...
void OcmeEditPlugin::mouseReleaseEvent(QMouseEvent * event, MeshModel &/*m*/, GLArea * gla)
{
	gla->update();
	cur=event->pos();
	haveToPick=true;
...
void OcmeEditPlugin::Decorate(MeshModel &m, GLArea * gla)
{
//	if(haveToPick)
//	{
//		glPushMatrix();
...
void OcmeEditPlugin::drawFace(CMeshO::FacePointer fp, MeshModel &m, GLArea * gla)
{
...
bool OcmeEditPlugin::StartEdit(MeshModel &/*m*/, GLArea *gla )
{
	QFrame * ocme_frame = new QFrame(gla->window());
	OcmeBox * ocmebox = new OcmeBox(ocme_frame);
//	dock->setAllowedAreas(Qt::NoDockWidgetArea);

//	QPoint p=parent->mapToGlobal(QPoint(0,0));
//	dock->setGeometry(5 + p.x(), p.y() + 5 , paintbox->width(), parent->height() - 10);

	ocmebox->setVisible(true);
