		...
		parentmultiview(mvcont)
{
	...
	connect(
		this->md(),
		SIGNAL(currentMeshChanged(int)),
		this,
		SLOT(manageCurrentMeshChange()),
		Qt::QueuedConnection);
	// connect(this->md(), SIGNAL(meshDocumentModified()), this,
	// SLOT(updateAllPerMeshDecorators()),Qt::QueuedConnection);
	 ...
	 *note as soon as the GLArea is added as Window to the QWorkspace the parent of GLArea is a
	 *QWidget, which takes care about the window frame (its parent is the QWorkspace again).
	 */
	...
	if (mainwindow != NULL) {
		...
		connect(
			mainwindow,
			SIGNAL(dispatchCustomSettings(const RichParameterList&)),
			this,
			SLOT(updateCustomSettingValues(const RichParameterList&)));
	}
	else {
...
const QAction *GLArea::getLastAppliedFilter()
{
	return lastFilterRef;
}

void GLArea::setLastAppliedFilter(const QAction *qa)
{
	lastFilterRef = qa;
}
...
void GLArea::drawGradient()
{
	...
	if (!takeSnapTile) {
		...
		glVertex2f(-1, 1);
		glColor(glas.backgroundBotColor);
		glVertex2f(-1, -1);
		glColor(glas.backgroundTopColor);
		glVertex2f(1, 1);
		glColor(glas.backgroundBotColor);
...
void GLArea::paintEvent(QPaintEvent* /*event*/)
{
...
#ifdef Q_OS_MAC
	glGetError(); // This seems required because on osx painter.beginNativePainting() trashes the
				  // err state of opengl
#endif
			...
			if (shared != NULL) {
				...
				for (MeshModel& mp : md()->meshIterator()) {
					if (meshVisibilityMap[mp.id()]) {
						...
						for (QList<QAction*>::iterator it = tmpset.begin(); it != tmpset.end();
							 ++it) {
							DecoratePlugin* decorInterface =
								qobject_cast<DecoratePlugin*>((*it)->parent());
							decorInterface->decorateMesh(
								*it,
								mp,
								this->glas.currentGlobalParamSet,
								this,
								&painter,
								md()->Log);
						}
				...
				if (meshVisibilityMap[mp.id()]) {
					...
					for (QList<QAction*>::iterator it = tmpset.begin(); it != tmpset.end(); ++it) {
						DecoratePlugin* decorInterface =
							qobject_cast<DecoratePlugin*>((*it)->parent());
						decorInterface->decorateMesh(
							*it, mp, this->glas.currentGlobalParamSet, this, &painter, md()->Log);
					}
				}
			}
		}
	...
	glPopMatrix(); // We restore the state to immediately after the trackball (and before the bbox
				   // scaling/translating)
	...
	foreach (QAction* p, iPerDocDecoratorlist) {
		...
		decorInterface->decorateDoc(
			p, *this->md(), this->glas.currentGlobalParamSet, this, &painter, md()->Log);
	}
	...
	if (hasToPick && !hasToGetPickPos) {
		...
		if (Pick<Point3f>(pointToPick[0], pointToPick[1], pp)) {
			...
			Logf(
				0,
				"Recentering on point [%f %f %f] [%d,%d]",
				pp[0],
				pp[1],
				pp[2],
				pointToPick[0],
...
void GLArea::displayMatrix(QPainter* painter, QRect areaRect)
{
	...
	for (int i = 0; i < 4; i++)
		tableText += QString("\t%1\t%2\t%3\t%4\n")
						 .arg(mm()->cm.Tr[i][0], 5, 'f', precision)
						 .arg(mm()->cm.Tr[i][1], 5, 'f', precision)
						 .arg(mm()->cm.Tr[i][2], 5, 'f', precision)
						 .arg(mm()->cm.Tr[i][3], 5, 'f', precision);
...
void GLArea::displayRealTimeLog(QPainter* painter)
{
	...
	// mQMultiMap<QString,std::pair<QString,QString> >::const_iterator it =
	// md()->Log.RealTimeLogText.constBegin();it != md()->Log.RealTimeLogText.constEnd();++it)
	for (QString keyIt : md()->Log.realTimeLogMultiMap().uniqueKeys()) {
		...
		// the meshname is used only to disambiguate when there are more than two boxes with the
		// same title
		for (const QPair<QString, QString>& itVal : valueList) {
			...
			doc.setHtml(
				"<font color=\"" + headColor.name() + "\" size=\"+1\" ><p><i><b>" + HeadName +
				"</b></i></p></font>"
				"<font color=\"" +
				textColor.name() + "\"             >" + itVal.second + "</font>");
...
void GLArea::displayInfo(QPainter* painter)
{
	...
	QRect Column_0(
		width() / 10, this->height() - barHeight + border, width() / 2, this->height() - border);
	QRect Column_1(
		width() / 2, this->height() - barHeight + border, width() * 3 / 4, this->height() - border);
	QRect Column_2(
		width() * 3 / 4, this->height() - barHeight + border, width(), this->height() - border);
	...
	painter->fillRect(
		QRect(0, this->height() - barHeight, width(), this->height()),
		ColorConverter::ToQColor(logAreaColor));
	...
	if (this->md()->meshNumber() > 0) {
		if (this->md()->meshNumber() == 1) {
			...
			col1Text += "Vertices: " + engLocale.toString(mm()->cm.vn) + "    (" +
						engLocale.toString(this->md()->vn()) + ") \n";
			col1Text += "Faces: " + engLocale.toString(mm()->cm.fn) + "    (" +
						engLocale.toString(this->md()->fn()) + ") \n";
		}
		...
		QLocale engLocale(QLocale::English, QLocale::UnitedStates);
		col1Text +=
			"Selection: v: " + engLocale.toString(svn) + " f: " + engLocale.toString(sfn) + " \n";
...
void GLArea::manageCurrentMeshChange()
{
	...
	if (iEdit) {
		...
		iEdit->layerChanged(
			*this->md(), *lastModelEdited, this, parentmultiview->sharedDataContext());

		// now update the last model edited
		// TODO this is not the best design....   iEdit should maybe keep track of the model on its
		// own
...
void GLArea::updatePerMeshDecorators(int)
{
	update();
}
...
//	makeCurrent();
//
//     for (QMap<int, QList<QAction *> >::iterator i = iPerMeshDecoratorsListMap.begin(); i !=
//     iPerMeshDecoratorsListMap.end(); ++i )
//     {
//
//         MeshModel *m = md()->getMesh(i.key());
//         foreach(QAction *p , i.value())
//         {
//             MeshDecorateInterface * decorInterface = qobject_cast<MeshDecorateInterface
//             *>(p->parent()); decorInterface->endDecorate  (p,
//             *m,this->glas.currentGlobalParamSet,this); decorInterface->setLog(&md()->Log);
//             decorInterface->startDecorate(p,*m, this->glas.currentGlobalParamSet,this);
//         }
//     }
//
...
void GLArea::setCurrentEditAction(QAction* editAction)
{
	...
	if (iEdit->isSingleMeshEdit()) {
		if (md()->mm() != NULL) {
			parentmultiview->sharedDataContext()->getRenderInfoPerMeshView(
				md()->mm()->id(), context(), dt);
			iEdit->suggestedRenderingData(*(md()->mm()), dt);
			MLPoliciesStandAloneFunctions::disableRedundatRenderingDataAccordingToPriorities(dt);
			parentmultiview->sharedDataContext()->setRenderingDataPerMeshView(
				md()->mm()->id(), context(), dt);
...
bool GLArea::readyToClose()
{
	...
	for (QMap<int, QList<QAction*>>::iterator it = iPerMeshDecoratorsListMap.begin();
		 it != iPerMeshDecoratorsListMap.end();
		 ++it) {
		...
		if ((iEdit != NULL) && !suspendedEditor) {
			...
			if (e->button() == Qt::RightButton) // Select a new current mesh
			{
				...
				if ((e->modifiers() & Qt::ShiftModifier) &&
					(e->modifiers() & Qt::ControlModifier) && (e->button() == Qt::LeftButton))
				...
				if (isDefaultTrackBall()) {
					...
					trackball.MouseDown(
						QT2VCG_X(this, e), QT2VCG_Y(this, e), QT2VCG(e->button(), e->modifiers()));
				}
				else
					trackball_light.MouseDown(
						QT2VCG_X(this, e), QT2VCG_Y(this, e), QT2VCG(e->button(), Qt::NoModifier));
			}
		}
	}
	update();
...
void GLArea::wheelEvent(QWheelEvent* e)
{
	...
	if ((iEdit && !suspendedEditor)) {
		...
		case Qt::AltModifier: {
			glas.pointSize = math::Clamp(
				glas.pointSize * powf(1.2f, notchX),
				MLPerViewGLOptions::minPointSize(),
				MLPerViewGLOptions::maxPointSize());
...
void GLArea::updateDecorator(QString name, bool toggle, bool stateToSet)
{
	...
	if (!iDecorateTemp) {
		this->Logf(
			GLLogStream::SYSTEM, "Could not get Decorate interface %s", qUtf8Printable(name));
	...
	if (iDecorateTemp->getDecorationClass(action) == DecoratePlugin::PerDocument) {
		...
		if (found) {
			if (toggle || stateToSet == false) {
				...
				this->Logf(
					GLLogStream::SYSTEM,
					"Disabled Decorate mode %s",
					qUtf8Printable(action->text()));
			}
			else
				this->Logf(
					GLLogStream::SYSTEM,
					"Trying to disable an already disabled Decorate mode %s",
					qUtf8Printable(action->text()));
		}
		else {
			if (toggle || stateToSet == true) {
				...
				bool ret =
					iDecorateTemp->startDecorate(action, *md(), glas.currentGlobalParamSet, this);
				if (ret) {
					this->iPerDocDecoratorlist.push_back(action);
					this->Logf(
						GLLogStream::SYSTEM,
						"Enabled Decorate mode %s",
						qUtf8Printable(action->text()));
				}
				else
					this->Logf(
						GLLogStream::SYSTEM,
						"Failed start of Decorate mode %s",
						qUtf8Printable(action->text()));
			}
			else
				this->Logf(
					GLLogStream::SYSTEM,
					"Trying to enable an already enabled Decorate mode %s",
					qUtf8Printable(action->text()));
		}
	}
	...
	if (iDecorateTemp->getDecorationClass(action) == DecoratePlugin::PerMesh) {
		...
		if (found) {
			if (toggle || stateToSet == false) {
				iDecorateTemp->endDecorate(
					action, currentMeshModel, glas.currentGlobalParamSet, this);
				iDecorateTemp->setLog(NULL);
			...
			if (toggle || stateToSet == true) {
				...
				if (iDecorateTemp->isDecorationApplicable(action, currentMeshModel, errorMessage)) {
					...
					bool ret = iDecorateTemp->startDecorate(
						action, currentMeshModel, glas.currentGlobalParamSet, this);
					if (ret) {
						...
						this->Logf(
							GLLogStream::SYSTEM,
							"Failed Decorate mode %s",
							qUtf8Printable(action->text()));
				}
				else
					this->Logf(
						GLLogStream::SYSTEM,
						"Error in Decorate mode %s: %s",
						qUtf8Printable(action->text()),
...
void GLArea::setLightingColors(const MLPerViewGLOptions& opts)
{
	...
	// WARNING!!!!! ALL THIS DATA SHOULD BE MOVED INSIDE THE OPTS....IN THE NEXT MESHLAB
	// RELEASE....MAYBE.....
...
void GLArea::setView()
{
	...
	glViewport(
		0,
		0,
		(GLsizei) QTLogicalToDevice(this, width()),
		(GLsizei) QTLogicalToDevice(this, height()));
	...
	if (fov <= 5)
		cameraDist = 8.0f; // small hack for orthographic projection where camera distance is rather
						   // meaningless...
	...
	float maxFarPlane =
		cameraDist +
		max(viewRatio(),
			float(-bb.min[2])); // is this guaranteed to be the largest interesting view?
	...
	// make sure far plane is always behind nearplane and avoid unnecessary wheel scrolling by
	// changing clip ratio
	...
	//    qDebug("Bb %f %f %f - %f %f %f", bb.min[0], bb.min[1], bb.min[2], bb.max[0], bb.max[1],
	//    bb.max[2]);
	...
	float /*cameraDist*/)
{
	...
	if (fovY <= 5) {
		...
		glOrtho(
			fLeft + tDimX * tileCol,
			fLeft + tDimX * (tileCol + 1), /* left, right */
			fBottom + tDimY * tileRow,
			fBottom + tDimY * (tileRow + 1), /* bottom, top */
			zNear,
			zFar);
	}
	else {
		...
		glFrustum(
			fLeft + tDimX * tileCol,
			fLeft + tDimX * (tileCol + 1),
			fBottom + tDimY * tileRow,
			fBottom + tDimY * (tileRow + 1),
			zNear,
			zFar);
	}
}
...
bool GLArea::isHelpVisible()
{
	return helpVisible;
}

bool GLArea::isTrackBallVisible()
{
	return trackBallVisible;
}

bool GLArea::isEditorSuspended()
{
	return suspendedEditor;
}

bool GLArea::isDefaultTrackBall()
{
...
void GLArea::sendViewPos(QString name)
{
	...
	Point3f pos =
		trackball.track.InverseMatrix() * Inverse(trackball.camera.model) * Point3f(0, 0, 0);
#else
	Point3f pos = Eigen::Transform3f(trackball.track.InverseMatrix()) *
...
void GLArea::copyToClip()
{
	viewToClipboard();
}

void GLArea::pasteFromClip()
{
	viewFromClipboard();
}
...
void GLArea::updateMeshSetVisibilities()
{
	...
	for (MeshModel& mp : md()->meshIterator()) {
		// Insert the new pair in the map; If the key is already in the map, its value will be
		// overwritten
	...
	for (RasterModel& rm : md()->rasterIterator()) {
		if (rm.id() == id) {
			...
			if (rm.currentPlane->image.isNull()) {
				Logf(
					0,
					"Image file %s has not been correctly loaded, a fake image is going to be "
					"shown.",
					rm.currentPlane->fullPathFileName.toUtf8().constData());
			...
			if (rm.shot.IsValid()) {
				...
				trackball.track.sca =
					3.0f / this->md()->bbox().Diag(); // hack, we reset the trackball scale factor
													  // to the size of the mesh object
				...
				trackball.track.sca = 1.0f / sceneCamSize; // hack, we reset the trackball scale
				factor to the size of the mesh object + viewpoint DOES NOT WORK !
				*/
...
void GLArea::initializeShot(Shotm& shot)
{
	...
	shot.Intrinsics.FocalMm =
		viewportYMm / (2 * tanf(vcg::math::ToRad(this->fovDefault() / 2.0f))); // 27.846098mm
...
{
	QString filename =
		QFileDialog::getOpenFileName(this, tr("Load Project"), "./", tr("Xml Files (*.xml)"));
...
bool GLArea::saveViewToFile()
{
	...
	if (!ret)
		QMessageBox::critical(
			this,
			tr("Meshlab Saving Error"),
			QString("Unable to save view file %1\n").arg(fileName));
...
void GLArea::loadShotFromTextAlignFile(const QDomDocument& doc)
{
	...
	while (!node.isNull()) {
		if (QString::compare(node.nodeName(), "Device") == 0) {
			...
			if (type == "GlImageWidget") {
				// Aligned Image
				if (QString::compare(node.attributes().namedItem("aligned").nodeValue(), "1") ==
					0) {
	...
	// this code seems useless, and if the camera translation is[0 0 0] (or even just with a small
	// z), there is a division by zero
	// Point3f p1 = rotFrom*(vcg::Point3f::Construct(rm->shot.Extrinsics.Tra()));
	// Point3f p2 = (Point3f(0,0,cameraDist));
	// trackball.track.sca =fabs(p2.Z()/p1.Z());
...
void GLArea::loadViewFromViewStateFile(const QDomDocument& doc)
{
	...
	while (!node.isNull()) {
		...
		{
		QDomNamedNodeMap attr = node.attributes();
		rm.drawMode = (vcg::GLW::DrawMode) (attr.namedItem("DrawMode").nodeValue().section('
		',0,0).toInt()); rm.colorMode = (vcg::GLW::ColorMode)
		(attr.namedItem("ColorMode").nodeValue().section(' ',0,0).toInt()); rm.textureMode =
		(vcg::GLW::TextureMode) (attr.namedItem("TextureMode").nodeValue().section('
		',0,0).toInt()); rm.lighting = (attr.namedItem("Lighting").nodeValue().section('
		',0,0).toInt() != 0); rm.backFaceCull =
		(attr.namedItem("BackFaceCull").nodeValue().section(' ',0,0).toInt() != 0);
		rm.doubleSideLighting = (attr.namedItem("DoubleSideLighting").nodeValue().section('
		',0,0).toInt() != 0); rm.fancyLighting =
		(attr.namedItem("FancyLighting").nodeValue().section(' ',0,0).toInt() != 0);
		}*/
		node = node.nextSibling();
	}
...
QPair<Shotm, float> GLArea::shotFromTrackball()
{
	...
	// add the translation introduced by gluLookAt() (0,0,cameraDist), in order to have the same
	// view--------------- T(gl)*S*R*T(t) => SR(gl+t) => S R (S^(-1)R^(-1)gl + t) Add translation
	// S^(-1) R^(-1)(gl) Shotd doesn't introduce scaling
	//---------------------------------------------------------------------
	shot.Extrinsics.SetTra(
		shot.Extrinsics.Tra() + (Inverse(shot.Extrinsics.Rot()) * Point3m(0, 0, cameraDist)));
...
void GLArea::loadShot(const QPair<Shotm, float>& shotAndScale)
{
	...
	////k is the ratio between the distances along z of two correspondent points (before and after
	///the translation) /from the point of view
	...
	// Matrix44f s0 = Matrix44f().SetScale(trackball.track.sca,trackball.track.sca,
	// trackball.track.sca); Matrix44f s1 = Matrix44f().SetScale(sca, sca, sca); Matrix44f r;
...
void GLArea::createOrthoView(QString dir)
{
	...
	view.Intrinsics.FocalMm =
		viewportYMm /
		(2 * tanf(vcg::math::ToRad(fov / 2))); // 27.846098 equivalente a circa 60 gradi
	...
	// scene uses "engineering" reference system, with Z as vertical axis
		...
		rot = Matrix44m().SetRotateDeg(90, Point3m(0, 1, 0)) *
			  Matrix44m().SetRotateDeg(90, Point3m(-1, 0, 0));
		...
		rot = Matrix44m().SetRotateDeg(90, Point3m(1, 0, 0)) *
			  Matrix44m().SetRotateDeg(180, Point3m(0, 1, 0));
	...
	// S^(-1) R^(-1)(gl) Shotd doesn't introduce scaling
	//---------------------------------------------------------------------
	view.Extrinsics.SetTra(
		view.Extrinsics.Tra() + (Inverse(view.Extrinsics.Rot()) * Point3m(0, 0, cameraDist)));
