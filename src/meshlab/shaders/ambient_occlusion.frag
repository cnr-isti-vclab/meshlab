...
void main(void)
{
   ...
   vec4 N = texture2D(nTexture, floor(gl_FragCoord.xy) / float(viewpSize-1));
   if (N.r < -1.2)
	   discard;

   vec4 P = project(texture2D(vTexture, floor(gl_FragCoord.xy) / float(viewpSize-1)));
   P=P*(viewpSize/texSize);
