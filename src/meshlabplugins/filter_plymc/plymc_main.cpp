  ...
  printf(
    ...
    " -F#     Use per vertex quality defined in plyfile (geodesic is disabled)\n"
	  ...
	  " -d#     set <n> as verbose level (default 0)\n"
		...
		"Subvolumes are ordered by Z, than by Y, then by X\n"
		"The parameter to be used is '-X' \n"
...
int main(int argc, char *argv[])
{
    ...
    //    case 'C' :	CM.MaxSize =atoi(argv[i]+2);printf("Setting MaxSize of MeshCache to %i\n",CM.MaxSize); break;
        ...
        case 's' :
            ...
            break;
        case 'S' :  p.IDiv[0]=atoi(argv[++i]);p.IDiv[1]=atoi(argv[++i]);p.IDiv[2]=atoi(argv[++i]);
