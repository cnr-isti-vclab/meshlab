...
void getInternFaces(MeshModel & m, vector<CMeshO::FacePointer> *actual,
		vector<CMeshO::VertexPointer> * risult,
		vector<CMeshO::FacePointer> * face_risult, GLArea * gla, Penn &pen,
		QPoint &current_point, QPoint &previous_point, GLfloat * pixels,
...
void EditSegment::mousePressEvent(QAction *, QMouseEvent *event, MeshModel & m,
		GLArea * gla) {
...
void EditSegment::mouseReleaseEvent(QAction *, QMouseEvent *event,
...
void EditSegment::mouseMoveEvent(QAction *, QMouseEvent *event, MeshModel & m,
		GLArea * gla) {
	//if (event->button() == Qt::MidButton) {
