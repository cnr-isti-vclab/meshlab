...
FilterMutualGlobalPlugin::FilterMutualGlobalPlugin()
{
...
QString FilterMutualGlobalPlugin::pluginName() const
{
	return "FilterMutualInfoPlugin";
}

QString FilterMutualGlobalPlugin::filterName(FilterIDType filterId) const
{
	switch(filterId) {
...
QString FilterMutualGlobalPlugin::filterInfo(FilterIDType filterId) const
{
	switch(filterId) {
	case FP_IMAGE_GLOBALIGN :
...
FilterMutualGlobalPlugin::FilterClass FilterMutualGlobalPlugin::getClass(QAction *a)
{
	switch(ID(a)) {
	case FP_IMAGE_GLOBALIGN :
...
MeshFilterInterface::FILTER_ARITY FilterMutualGlobalPlugin::filterArity(QAction*) const
{
	return SINGLE_MESH;
}

void FilterMutualGlobalPlugin::initParameterSet(QAction *action,MeshDocument & md, RichParameterSet & parlst)
{
...
bool FilterMutualGlobalPlugin::applyFilter(QAction *action, MeshDocument &md, RichParameterSet & par, vcg::CallBackPos* )
{
	switch(ID(action))	 {
	case FP_IMAGE_GLOBALIGN :
...
int FilterMutualGlobalPlugin::postCondition(QAction*) const
{
	return MeshModel::MM_NONE;
}

QString FilterMutualGlobalPlugin::filterScriptFunctionName(FilterIDType filterID)
{
	switch(filterID) {
	case FP_IMAGE_GLOBALIGN :
...
bool FilterMutualGlobalPlugin::imageGlobalAlign(
		MeshDocument &md,
		float thresDiff,
		bool preAlign,
		...
		int rendmode)
{
	...
	if (!initGLGlobalAlign()){
		Log(0, "Init GL failed.");
		return false;
	}

	if (preAlign) {
		preAlignment(md, estimateFocal, fine, rendmode);
	}
	...
	if (maxRefinementSteps!=0) {
		...
		for (int i=0; i<maxRefinementSteps; i++) {
			...
			Log(0, "AlignGlobal %d of %d completed, average improvement %f pixels", i+1, maxRefinementSteps, diff);
			if (diff<thresDiff)
				break;
			oldShots.clear();
			for (int r=0; r<md.rasterList.size();r++) {
...
bool FilterMutualGlobalPlugin::preAlignment(
		MeshDocument &md,
		bool estimateFocal,
		bool fine,
		int rendmode)
{
	...
	if (md.rasterList.size()==0) {
		...
		glBufferDataARB(
					GL_ARRAY_BUFFER_ARB, alignset.mesh->vn*sizeof(vcg::Point3f),
		...
		glBufferDataARB(
					GL_ARRAY_BUFFER_ARB, alignset.mesh->vn*sizeof(vcg::Color4b),
					colors, GL_STATIC_DRAW_ARB);
		glBindBufferARB(GL_ARRAY_BUFFER_ARB, 0);

		glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, alignset.ibo);
		glBufferDataARB(
					GL_ELEMENT_ARRAY_BUFFER_ARB, alignset.mesh->fn*3*sizeof(unsigned int),
...
float FilterMutualGlobalPlugin::calcShotsDifference(MeshDocument &md, std::vector<vcg::Shotf> oldShots, std::vector<vcg::Point3f> points)
{
...
bool FilterMutualGlobalPlugin::initGLGlobalAlign()
{
	...
	if (!glewIsSupported("GL_ARB_vertex_shader") || !glewIsSupported("GL_ARB_fragment_shader") ||
		...
		Log(0, "Warning: Graphics hardware does not fully support Shaders");
	}
...
std::vector<SubGraph> FilterMutualGlobalPlugin::buildGraph(MeshDocument &md, bool globalign)
{
...
std::vector<AlignPair> FilterMutualGlobalPlugin::calcPairs(MeshDocument &md, bool globalign)
{
	Solver solver;
	MutualInfo mutual;
...
std::vector<SubGraph> FilterMutualGlobalPlugin::createGraphs(MeshDocument &md, std::vector<AlignPair> arcs)
{
...
bool FilterMutualGlobalPlugin::alignGlobal(MeshDocument &md, std::vector<SubGraph> graphs)
{
	for (int j=0; j<1; j++) {
...
int FilterMutualGlobalPlugin::getTheRightNode(SubGraph graph)
{
...
bool FilterMutualGlobalPlugin::allActive(SubGraph graph)
{
	for (int k=0; k<graph.nodes.size(); k++) {
...
bool FilterMutualGlobalPlugin::alignNode(MeshDocument &md, Node node)
{
	Solver solver;
	MutualInfo mutual;
...
bool FilterMutualGlobalPlugin::updateGraph(MeshDocument &md, SubGraph graph, int n)
{
	Solver solver;
	MutualInfo mutual;
...
MESHLAB_PLUGIN_NAME_EXPORTER(FilterMutualGlobalPlugin)
