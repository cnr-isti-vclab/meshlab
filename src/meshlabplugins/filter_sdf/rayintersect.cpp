...
CFaceO* RayIntersectCell::intersects(const Ray3f& ray){
    // (CGAL::do_intersect(*(m_triangleVector[*it]), ray))
    float t,u,v;
    CFaceO* face;
...
void RayIntersect::Init(CMeshO& mesh, const int gridSize)
{
    ...
    // Step2 - modified! Run over triangles and add their triangles to the grid
    // coordinates2index(p, xindex, yindex, zindex);
    // m_grid[xindex][yindex][zindex]->addTriangle( fIdx );

	//step 4
	m_initialized = true;
    // generateReportFile("c:/temp/rayintersect.html");
}
