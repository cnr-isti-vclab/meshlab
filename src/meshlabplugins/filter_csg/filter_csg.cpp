...
std::map<std::string, QVariant> FilterCSG::applyFilter(const QAction *filter, const RichParameterList & par, MeshDocument &md, unsigned int& /*postConditionMask*/, vcg::CallBackPos *cb)
{
	...
	case FP_CSG:
	{
		...
		if ((firstMesh == NULL) || (secondMesh == NULL))
		{
			...
			throw MLException("cannot compute, mesh does not exist");
		}

		if ((firstMesh->cm.fn == 0) || (secondMesh->cm.fn == 0))
		{
			log("CSG filter: cannot compute, mesh has no faces");
			throw MLException("cannot compute, mesh has no faces");
		}
		...
		QString errorMsg;
		if (!isValid (firstMesh->cm, errorMsg) || !isValid (secondMesh->cm, errorMsg))
		...
		default:
			throw MLException("Wrong operation selected.");
		}
