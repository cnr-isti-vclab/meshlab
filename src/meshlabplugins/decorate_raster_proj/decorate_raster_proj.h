...
private:
	...
	// Member functions.
	void updateCurrentMesh(MeshDocument& m, const RichParameterList& par);
	void
	updateCurrentRaster(MeshDocument& m, QOpenGLContext* glctx, MLSceneGLSharedDataContext* ctx);

	void setPointParameters(MeshDrawer& md, const RichParameterList* par);
	void drawScene();
