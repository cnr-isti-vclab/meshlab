...
Parameters::Parameters(bool _use_focal, vcg::Shot<float> &_reference, int w, int h, vcg::Box3<float> &_box,
...
void Parameters::initScale(CMeshO &mesh, int nsamples) {
  ...
  for(int i = 0; i < size(); i++) {
    p[i] = step;
    vcg::Shot<float> test = toShot(false);
...
//}
//
double Parameters::pixelDiff(vcg::Shot<float> &test, CMeshO &mesh, int nsamples) {
  double maxdist = 0.0;
  double avedist = 0.0;
  int count = 0;
...
//}
//
vcg::Point2f Parameters::pixelDiff(vcg::Shot<float> &test, vcg::Point3f p) {
