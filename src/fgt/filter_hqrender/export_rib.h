...
static int Save(SaveMeshType &m,  const char * filename, bool binary, CallBackPos *cb=0) {
  return Save(m,filename, 0, 1.0, binary, cb);
}

static int Save(SaveMeshType &m,  const char * filename, int savemask, bool binary, CallBackPos *cb=0) {
  return Save(m,filename, savemask, 1.0, binary, cb);
}

static int Save(SaveMeshType &m,  const char * filename, float scale, bool binary, CallBackPos *cb=0) {
  return Save(m,filename, 0, scale, binary, cb);
}

static int Save(SaveMeshType &m,  const char * filename, int savemask, float scale, bool binary, CallBackPos *cb=0)
{
  ...
  if((m.HasPerVertexTexCoord() && (savemask & Mask::IOM_VERTTEXCOORD)) || 
	  (m.HasPerWedgeTexCoord() && (savemask & Mask::IOM_WEDGTEXCOORD)))
    fprintf(fout,"Declare \"st\" \"facevarying float[2]\"\n");
  
  if(m.HasPerVertexNormal() && (savemask & Mask::IOM_VERTNORMAL))
  ...
  if(m.HasPerVertexTexCoord() || m.HasPerWedgeTexCoord())
	fprintf(fout,"Declare \"st\" \"facevarying float[2]\"\n");

  if(m.HasPerWedgeTexCoord() || m.HasPerVertexTexCoord())
	fprintf(fout,"Declare \"st\" \"facevarying float[2]\"\n");
  ...
  Matrix44f transf;
  if(scale != 1.0) {
    Point3f centerBB = m.trBB().Center();
    ...
    transf = scaleMatrix * translateMatrix;
  }
