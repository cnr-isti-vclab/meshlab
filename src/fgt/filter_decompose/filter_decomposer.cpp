...
class CutVertex  : public vcg::Vertex<CutUsedTypes, vcg::vertex::Coord3f, vcg::vertex::Normal3f, vcg::vertex::VFAdj, vcg::vertex::Qualityf, vcg::vertex::Color4b, vcg::vertex::BitFlags  >{};
class CutFace    : public vcg::Face<CutUsedTypes, vcg::face::VFAdj, vcg::face::VertexRef, vcg::face::VFAdj, vcg::face::FFAdj, vcg::face::FEAdj, vcg::face::Qualityf, vcg::face::Normal3f, vcg::face::Color4b, vcg::face::BitFlags > {};
...
class WeightFunctor{
private: float _ambient_weight, _dihedral_weight, _elength, _geo, _max1, _max2, _maxq, _geomin, _geomax; CutFace::FacePointer _start,_end;
...
public:
    ...
    */
    double operator() (vcg::face::Pos<CutFace> pos)
    {
        return ((_elength * vcg::Norm(pos.V()->P() - pos.VFlip()->P())) + ( _geo * GeodesicDistance(pos)) + (_ambient_weight * (pos.f->Q()+pos.FFlip()->Q())/2) + (_dihedral_weight * AngleDistance(pos)));
    }

    double GeodesicDistance(vcg::face::Pos<CutFace> pos)
    {
    ...
    double AngleDistance(vcg::face::Pos<CutFace> pos){
        return (M_PI + vcg::face::DihedralAngleRad<CutFace>(*(pos.f), pos.E()))/2*M_PI;
...
/*utility to get the closest faces to the parameters points --(to be removed)---*/
static void GetClosestFace(CutMesh &cm, vcg::Point3f pointStart, vcg::Point3f pointEnd, CutMesh::FacePointer &startFace, CutMesh::FacePointer &endFace)
{
    //get the closest face to the upper and lower point
    ...
    {
        vcg::Point3f avg = ((&*fit)->V(0)->P()+(&*fit)->V(1)->P()+(&*fit)->V(2)->P())/3;
        if(vcg::SquaredDistance(pointStart, avg) <= minDistStart)
        {
            startFace = &*fit;
            minDistStart=vcg::SquaredDistance(pointStart,avg);
        }
        else if(vcg::SquaredDistance(pointEnd, avg) <= minDistEnd)
        {
            endFace = &*fit;
            minDistEnd = vcg::SquaredDistance(pointEnd, avg);
        }
    }
    return;
}
...
bool ExtraSamplePlugin::applyFilter(QAction *action, MeshDocument &md, RichParameterSet & par, vcg::CallBackPos *cb)
{
	switch(ID(action))	 {
        ...
        case FP_DECOMPOSER :
        {
            ...
            GetClosestFace(cm,upperPoint, lowerPoint, startFace, endFace);
